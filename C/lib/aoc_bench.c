...
void timer_func_bench(int part, void *(func)(AocData_t *), AocData_t *aocdata, int show_res,
                      unsigned warmup, unsigned iterations) {
    char *result = NULL;
#ifdef _WIN32
    LARGE_INTEGER freq;
    ...
    for (unsigned warmup_counter = 0; warmup_counter < warmup; warmup_counter++) {
        ...
        double timeDifference =
            (stop.tv_sec * 1e9 + stop.tv_nsec) - (start.tv_sec * 1e9 + start.tv_nsec);
#endif
    ...
    if (show_res) {
        printf("Part %d answer: %-20s%10.2lf %-2s (+/- %.2lf %s)\n", part, result,
               duration.duration, duration.unit, stddevduration.duration, stddevduration.unit);
    } else {
