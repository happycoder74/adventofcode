...
AocArrayPtr clean_input(AocArrayPtr data) {
    ...
    rucksacks = aoc_array_new(sizeof(AocArrayPtr));
    for (uint32_t i = 0; i < aoc_array_length(data); i++) {
        ...
        memcpy(pack1, row, sizeof(char) * pack_length);
        pack1[pack_length] = '\0';
        memcpy(pack2, ptr, sizeof(char) * pack_length);
        pack2[pack_length] = '\0';
        ...
        aoc_array_append(rucksacks, packs);
    }
...
void *solve_part_1(AocData_t *data) {
    ...
    for (uint16_t i = 0; i < aoc_data_length(data); i++) {
        packs = aoc_array_index(aoc_data_get(data), i);
...
void *solve_part_2(AocData_t *data) {
    ...
    for (uint32_t i = 0; i < aoc_array_length(groups); i++) {
        group = aoc_array_index(groups, i);
